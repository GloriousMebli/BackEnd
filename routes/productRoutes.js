...
const {
  ...
  deleteImage,
  backBlazeInit,
} = require("../functions/upload"); // Adjust the path if needed
...
router.get("", async (req, res) => {
  try {
    ...
    // Отримуємо всі продукти відповідно до критеріїв (без пагінації на цьому етапі)
      ...
      products = products.map((product) => {
        ...
        if (product.price && typeof product.price === "string") {
          numericPrice = parseFloat(
            product.price.replace(/\s|грн|ГРН|Грн|UAH|uah|Uah|\$|₴/g, "")
          );
        }
    ...
    // Застосовуємо сортування до всіх продуктів
    if (query.sortBy === "price") {
      const order = query.order === "desc" ? -1 : 1;
      products.sort((a, b) => order * (a.numericPrice - b.numericPrice));
    } else if (query.sortBy === "createdAt") {
      const order = query.order === "desc" ? -1 : 1;
      products.sort(
        (a, b) => order * (new Date(b.createdAt) - new Date(a.createdAt))
      );
    }
    ...
    // Застосовуємо пагінацію до відсортованих продуктів
    const paginatedProducts = products.slice(skip, skip + limit);

    // Підрахунок загальної кількості продуктів для фронтенду
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const thumbnailUrl =
        config.publicUrl + data?.thumbnailResponse?.data?.fileName;
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const currentImage = product.images.find((image) =>
